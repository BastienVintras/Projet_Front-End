...
export const Button = ({
  ...
  switch (variant) {
    ...
    case "secondary":
      variantStyles =
        "bg-primary-200 hover:bg-primary-300/50 text-primary rounded"; //le /50 est la valeur de l opacité
  ...
  const buttonElement = (
    <button
      ...
      className={clsx(
        ...
        fullWith && "w-full",
        "relative animate"
      )} //la position absolute du spinner est en fonction de la position relative dans le corp du bouton
      onClick={handleClick}
      disabled={disabled || isLoading || isLoading ? true : false}
    >
      {buttonContent}
    </button>
  );
